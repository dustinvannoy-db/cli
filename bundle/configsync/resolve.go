...
// elementInOverrideBlock reports whether a merged sequence element was contributed
// by a target override block rather than the top-level resources block. Target
// overrides are merged with the top-level block as the merge reference, and merge
// keeps the reference locations first, so seqLocations[0] is always the top-level
// anchor regardless of where each block appears in the file. An element whose
// nearest block anchor is not that top-level anchor came from a target block, so
// its block-local index is only valid under the targets.<t>. prefix.
//
// A resource may only be declared once across all top-level files (a duplicate
// key is rejected at load), so a second sequence location can only come from a
// target override — never from a second top-level file.
...
// in-memory index may not match the position in the YAML file.
//
// A single field can hold sequences from more than one config block — e.g. a job's
// tasks defined partly under top-level `resources.jobs.<job>.tasks` and partly under
// `targets.<t>.resources.jobs.<job>.tasks`. MergeJobTasks concatenates and sorts them
// into one in-memory sequence, but each block is patched independently in the YAML.
// seqLocations holds one location per contributing block (the location of each block's
// sequence node); an element belongs to the block whose anchor is the closest one at or
// before it in the same file. Counting only same-block elements with a smaller line
// gives the index within that block, which is what the YAML patch targets. When the
// sequence comes from a single block this reduces to a plain same-file count.
	...
	for _, resourceKey := range resourceKeys {
		...
		for _, fieldPath := range fieldPaths {
			...
			if configChange.Operation == OperationAdd && resolvedPath.BracketStar() {
				parentKey := blockScopedParent(resolvedPath.Parent().String(), targetName, inOverrideBlock)
				indices, ok := indicesToReplaceMap[parentKey]
				if ok && len(indices) > 0 {
					index := indices[0]
					indicesToReplaceMap[parentKey] = indices[1:]
			...
			case inOverrideBlock:
				// The index is local to the target override block, so only the
				// target-prefixed candidate points at the right element. Emitting the
				// unprefixed candidate too would let applyChange write the same index
				// into the top-level block and silently patch the wrong element.

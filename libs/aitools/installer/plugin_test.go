...
func TestInstallBuiltinMarketplaceRecoversAfterPrompt(t *testing.T) {
	...
	assert.Contains(t, cmds, "claude plugin marketplace update claude-plugins-official")
...
func TestInstallBuiltinMarketplaceUserDeclinesPrompt(t *testing.T) {
	...
	for _, c := range stub.Commands() {
		assert.NotContains(t, c, "marketplace add", "declining must not touch the marketplace")
		assert.NotContains(t, c, "marketplace update", "declining must not touch the marketplace")
	}
}
...
func TestInstallBuiltinMarketplaceNonInteractiveActionableError(t *testing.T) {
	...
	for _, c := range stub.Commands() {
		...
		assert.NotContains(t, c, "marketplace update")
	}
}
